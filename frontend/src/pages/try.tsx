...
export function TryPage() {
  const [step, setStep]         = useState<'role'|'goal'|'experience'|'signup'>('role')
  ...
  const stepNum  = step === 'role' ? 1 : step === 'goal' ? 2 : step === 'experience' ? 3 : 4
      ...
      <div className={cn('w-full max-w-lg transition-opacity duration-180', anim ? 'opacity-0' : 'opacity-100')}>
        ...
        {step === 'goal' && role && (
          <div>
            ...
            <button disabled={!goalId} onClick={() => go('experience')}
              className={cn(
                'w-full py-4 rounded-2xl text-sm font-bold transition-all flex items-center justify-center gap-2',
                goalId
                  ? 'bg-[#5855D6] text-white hover:bg-[#4744C8] shadow-lg shadow-[#5855D6]/20'
        ...
        {/* ── Step 3: Experience ──────────────────────────────────────────── */}
        {step === 'experience' && role && goalId && (
          <TryExperienceStep
            experience={getExperience(role, goalId)}
            onDone={goSignup}
            onBack={() => go('goal')}
          />
        )}
